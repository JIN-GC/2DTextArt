...
public class CheckAccess {
	...
	public static BufferedImage isUrl(String str) {
		...
		try {
			...
			//	URLパターン判定
			URL url = new URL(str);
			//	String pattern = "^(http?)://.*$|^data:.*$";
				String pattern = "^(http|https|ftp|ftps|sftp|scp)://.*$|^data:.+";
			//	String pattern = "^(https?|ftp|ftps|sftp|scp)://.*$|^data:.+";
	...
	//	MIME状況確認処理
	static BufferedImage checkMime(File file){
		try {
			...
			//	so.printf("\033[1mEXT: \033[42m%5s\033[0m\033[1m; MIME: \033[42m%10s\033[0m\n",ext.substring(ext.lastIndexOf(".") + 1), mime);
			//	形式判別
			...
			||  (mime == "image/gif"  || ext.endsWith(".GIF"))){
				...
				System.out.printf("\033[1mEXT: \033[41m%5s\033[0m\033[1m; MIME: \033[41m%10s\033[0m\033[1m  \033[31m処理対象外\033[0m\n\n",ext.substring(ext.lastIndexOf(".") + 1), mime);
				return null;
			}
		} catch (IOException e) {
			e.printStackTrace();
